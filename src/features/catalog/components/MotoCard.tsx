...
/**
 * MotoCard – Uniforme, parejo y GRANDE (sin reacomodo al cargar)
 * - Marco 4:3 fijo
 * - Padding fijo (evita “saltos” de tamaño cuando termina de cargar)
 * - AutoCropImage recorta/escala y auto-boosta si quedó chica
 */
export function MotoCard({ moto }: { moto: MotoItem }) {
  const [src] = useState(moto.img || "/motos/_no-image.png");

  // ✅ Fijo: evita el reacomodo y mantiene tamaño parejo
  // Más chico = imagen más grande
  const autoPad = 0.03;
  ...
  return (
    ...
    >
      ...
      {/* IMAGEN — marco fijo 4:3, recorte + auto-boost (sin salto) */}
      <div className="relative w-full aspect-[4/3] bg-white border-b border-neutral-200 overflow-hidden flex items-center justify-center">
        <Img
          src={src}
          alt={moto.nombre}
          padding={autoPad}        // ✅ constante → tamaño parejo
          minFillPct={1.06}        // ✅ más grande desde el inicio
          whiteTolerance={245}
          aggressive={true}
          autoBoostSize={true}
