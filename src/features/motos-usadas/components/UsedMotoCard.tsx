...
function Spec({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="text-[14px] font-medium text-neutral-900 leading-snug">
        {value}
      </div>
    </div>
  );
...
export function UsedMotoCard({ moto }: { moto: UsedMotoItem }) {
  const images = moto.imagenes?.filter(Boolean) ?? [];
  const [active, setActive] = useState(0);

  const hasImages = images.length > 0;
  const mainSrc = useMemo(() => {
    if (!hasImages) return "";
    return images[active] || images[0] || "";
  }, [active, images]);
  ...
  return (
    ...
    >
      ...
      <div className="bg-white border-b border-neutral-200 overflow-hidden">
        <div className="relative w-full aspect-[4/3] flex items-center justify-center bg-white">
          {hasImages ? (
            <img
              src={mainSrc}
              alt={moto.nombre}
            ...
            <div className="w-full h-full flex flex-col items-center justify-center px-4 text-neutral-500">
              <div className="text-[12px] font-semibold mb-2">Sin fotos cargadas</div>
              <div className="text-[12px] text-neutral-400 text-center leading-relaxed">
                Actualizá el stock con imágenes para que se vea la galería.
              </div>
            </div>
          )}
        </div>
        ...
        {hasImages && images.length > 1 && (
          <div className="px-3 py-2 sm:px-4 bg-white">
            <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:thin]">
              {images.map((src, idx) => (
                <button
                  ...
                  className={[
                    "shrink-0 w-14 h-10 sm:w-16 sm:h-11 rounded border transition",
                    idx === active
                ...
                >
                  <img
                    src={src}
                    alt={`${moto.nombre} - miniatura ${idx + 1}`}
      ...
      <div className="px-4 sm:px-5 py-4 sm:py-5 bg-white">
        <div className="grid grid-cols-2 gap-x-6 gap-y-3">
          <Spec label="AÑO" value={moto.anio || "—"} />
          <Spec label="KM" value={moto.km || "—"} />
          <Spec label="PRECIO" value={moto.precio || "—"} />
          <Spec label="ESTADO" value={moto.estado || "—"} />

          <div className="col-span-2">
            <Spec
              label="DOCUMENTACIÓN"
              value={moto.documentacionOk ? "Sí" : "No"}
            />
          </div>

          <div className="col-span-2 sm:col-span-1">
            <Spec label="TRANSMISIÓN" value={moto.transmision ?? "—"} />
          </div>
          <div className="col-span-2 sm:col-span-1">
            <Spec label="CILINDRADA" value={moto.cilindrada ?? "—"} />
          </div>

          {moto.potencia && (
            <div className="col-span-2">
              <Spec label="POTENCIA" value={moto.potencia ?? "—"} />
            </div>
          )}
        </div>
      </div>
      ...
      {/* WHATSAPP */}
      {/*
        Armamos el mensaje para que no muestre líneas vacías cuando aún no
        están cargados año/km/precio.
      */}
      <a
        href={`https://wa.me/5493795134533?text=${encodeURIComponent(
          [
            "Hola! Quiero consultar motos usadas:",
            `- ${moto.nombre}`,
            `- ${moto.marca} ${moto.tipo}`,
            moto.anio || moto.km
              ? `- ${moto.anio || "—"}${moto.km ? ` (${moto.km})` : ""}`
              : null,
            moto.precio ? `Precio: ${moto.precio}` : null,
          ]
            .filter(Boolean)
            .join("\n")
        )}`}
        ...
        "
        aria-label={`Consultar ${moto.nombre} por WhatsApp`}
      >
        <svg
          viewBox="0 0 24 24"
          fill="currentColor"
